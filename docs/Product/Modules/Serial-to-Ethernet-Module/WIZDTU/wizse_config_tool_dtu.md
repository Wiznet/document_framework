...
WIZS2E Config Tool is an upper computer configuration software that runs on the Windows operating system and is compatible with WIZ-DTU, WIZ-IP32, W5500S2E and W7500S2E series serial to Ethernet modules. Users can easily search, view, and configure various functions and information of WIZ-DTU serial to Ethernet converter through WIZS2E Config Tool.
...
The WIZ-DTU converter includes Basic Settings and Channel1 configurations.

The following are the list for all configurables for WIZ-DTU converter.
...
| --------------------------------------- | ----------------------------------| ------------------------- |
| Baud Rate                        		  | 1200 bps ~ 1,152,000 bps          | 115,200        			  |
...
<a name="mod"></a>
| [Modbus](#modbus) | Description                	 | Default                   |
| ----------------- | ------------------------------ | ------------------------- |
| Enable Modbus 	| Enable Modbus function         | Disable (uncheck)      	 |
| Work as       	| "Modbus RTU" or "Modbus ASCII" | Modbus RTU (locked)       | 
...
The following is an example of the converter's network interface selection List 
|                                                  |
| ----------------------------------------------   |
...
<span style={{ color: 'red' }}>**Note:** The firmware must be the officially provided [Application firmware](../WIZDTU#software-resources) for the W5500S2E / W7500S2E series from WIZnet.</span> 
...
As shown in the following figure, these are the basic parameters for WIZ-DTU, which is divided into two parts for explanation.
...
- **Embedded Web Server Port** - Set/Display the port number of the Web server of WIZ-DTU. **The default is 80.** It is not recommended to change it. The range of values is 0 to 65,535. If it is not set to 80, then it is required to add the web server port to the browser address bar. For example: 192.168.1.88:8000.
...
As shown in the following figure, these are the TCP and Serial setups for WIZ-DTU, which is divided into four parts for explanation.
...
- **Enable Modbus** - Checkbox to enable/disable the Modbus Feature for WIZ-DTU. **The default is disabled (unchecked)**.

- **Work as** - The list box after the Modbus enable selection. It allows user to choose the Modbus to operate in "Modbus RTU" or **"Modbus ASCII". The default is "Modbus RTU" and it is disabled (unchecked)**. 
