...
# WIZ-DTU Web Configuration

WIZ-DTU  supports web page configuration. We recommend using Chrome or Firefox. Other browsers may have display or functional issues. Let's take Chrome browser as an example to illustrate.

Before using web configuration, it is necessary to ensure that WIZ-DTU  can be accessed correctly. That is, if configuring within a local area network, WIZ-DTU  needs to be on the same network segment as the computer. If configuring remotely, WIZ-DTU  needs to be mapped to a public IP. Taking the local area network as an example for configuration, the specific configuration method is introduced as follows.
...
Since WIZ-DTU has two serial channels, it has two identical setting pages for both channels. For distinguish both channels, please find the table below.
...
| ------------------------------------- | -------------------------------- | ---------------------------------------------------------- |
| Current Firmware Version              | Display latest firmware version  | Check [Software Resource](../WIZDTU#software-resources)    |
...
The default login password is “**admin**”. Click "Login" to log in and enter the WIZ-DTU  homepage. It should be noted that after logging into the WIZ-DTU  webpage, in order to ensure the security of module information, if the user does not take any action within 5 minutes, they need to log in again, as shown in the following figure.
...
As shown in the figure below, it is the basic information page after logging in to the WIZ-DTU  configuration page. If you need to log out, click the "Logout" link in the upper right corner. This page provides an overview of the basic configuration information for WIZ-DTU .

|                                                   |
| ------------------------------------------------- |
| ![](/img/products/wizdtu/web_device_overview.png) |
...
As shown in the following figure, it is the WIZ-DTU  basic parameter configuration page,which is divided into following parts for explanation.
...
**Use NETBIOS:** Enable the NetBIOS function option. When checked, the NetBIOS function will be enabled. **By default, it is enabled.** After enabling the NetBIOS function, users can directly access the built-in web page of WIZ-DTU  by entering "http:// module name" in the browser address bar.

**HTTP Port:** Set/Display the port number of the Web server of WIZ-DTU . **The default is 80.** It is not recommended to change it. The range of values is 0 to 65,535. If not set to 80, then in the browser address bar, it should be entered as "IP:Port", for example: 192.168.1.88:8000.
...
As shown in the figure below, it is Channel 1's configuration page of WIZ-DTU . The following is a detailed explanation:
...
After the firmware upgrade is completed, WIZ-DTU module will automatically restart to complete the firmware upgrade and the webpage will automatically redirect to the login interface.
