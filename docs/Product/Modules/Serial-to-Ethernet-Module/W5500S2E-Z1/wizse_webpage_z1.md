...
| ------------------------------------- | -------------------------------- | ---------------------------------------------------------- |
| Current Firmware Version              | Display latest firmware version  | Check [Software Resource](../W5500S2E-Z1#software-resources) |
...
Open the Chrome browser, enter the IP address of W5500S2E series module in the address bar (the factory default is 192.168.1.88), and the login interface shown in the ﬁgure below will appear.
...
The default login password is “**admin**”. Click "Login" to log in and enter the W5500S2E series homepage. It should be noted that after logging into the W5500S2E series webpage, in order to ensure the security of module information, if the user does not take any action within 5 minutes, they need to log in again, as shown in the following figure.
...
As shown in the figure below, it is the basic information page after logging in to the W5500S2E series configuration page. If you need to log out, click the "Logout" link in the upper right corner. This page provides an overview of the basic configuration information for W5500S2E series.
...
As shown in the following figure, it is the W5500S2E series basic parameter configuration page,which is divided into following parts for explanation.
...
- **Start Mode:** Device startup mode configuration, which can be set to AT command mode and data transmission mode. The W5500S2E series will operate in this mode upon the next power-on/reboot. **The default is AT command mode.**
...
<span style={{ color: 'red' }}>**Note:** Some protocols in the Ethernet protocol have default port numbers. These port numbers should be avoided. The default occupied port numbers can be found in the </span> [appendix](wizse_at_command_z1#appendix).
...
- **Select Modbus Mode:** In W5500S2E Series, it includes Modbus RTU and Modbus ASCII formats to let the user to communicate using Modbus TCP. **The default is Modbus RTU.**
...
As shown in the figure below, it is the advanced configuration page of W5500S2E series. The following is a detailed explanation:
...
|                                                         |
| ------------------------------------------------------- |
| ![](/img/products/w5500s2e-z1/web_firmware_upgrade.png) |
...
**Logout:** Logout Button to logout of the W5500S2E series configuration page. It shows the following window, click "Ok' will jump to login page.
