...
| 9          | UART0TX / SPI_MOSI | Serial data TX / MOSI pin for Data SPI transmission |
| 10         | UART0RX / SPI_CSn  | Serial data RX / SPI Chip Select pin        |
| 11         | UART0_CTS   | Clear To Send (RX)<br />Associated with the RTS of the connected device and is used to transmit data when the RTS sent from that pin is low.                                                                                                                                            |
| 12         | UART0_RTS   | Request To Send<br />Connects with the CTS of the connected equipment and sends a low signal when the W55RP20-S2E is in a state where it can receive data.<br /><br />When 485/422 selected by UART_IF_SEL pin, this pin act as 485/422 select pin. <br />NC : 485<br />Low : 422<br /> |
