...
- WIZnetの「シリアルからイーサネットへ（Serial to Ethernet）」製品は、シリアル機器から送信されたデータをTCP/IPデータとして転送し、ネットワークを通じて受信したTCP/IPデータを再びシリアルデータに変換してシリアル機器へ送信するプロトコルコンバーターです。

- 本製品を使用することで、**UART/SPIインターフェースをサポートするデバイスに簡単にイーサネットネットワーク機能を追加**でき、デバイスをイーサネットネットワーク（インターネット）経由で制御することが可能になります。

- また、顧客の要望に応じて機能の変更・追加を行う **[カスタマイズサービス](#カスタマイズサービス)** も提供しています。
...
| 9 | UART1_TX / SPI_MOSI | シリアルデータ送信（TX）/ データSPI送信用MOSIピン |
| 10 | UART1_RX / SPI_CSn | シリアルデータ受信（RX）/ SPIチップセレクトピン |
...
| 19 | UART_SPI_SEL | UART/SPIインターフェース選択ピン<br />High : SPI<br />LowまたはNC : UART |
...
| 65 | UART0_TX | デバッグTX |
| 66 | UART0_RX | デバッグRX |
| 67 | SPI_SCK | データSPI送信用SCK入力ピン（SPIモード） |
| 68 | SPI_MISO | データSPI受信用MISOピン（SPIモード） |
| 69 | GND | グラウンド |
