...
** Supported Languages **  
* [English](/products/s2e_module/wiz752sr-120/commandmanual/en) (current page)  
...
-   **2. Enter command mode using command mode switch code**[^3]
    ...
    -   The command mode switch code is composed of 3-byte Hex codes,
        ...
        accepts Hex value. (**default: Hex \[2B\]\[2B\]\[2B**\][^4])
      ...
      - **Use [[#ex|EX]] command to switch to data transmission mode.**
        ...
        * <code>> SEG:GW Mode </code>**Please be cautious when using the trigger code to switch command
mode.**
...
following commands. The user must use **UDP** or **TCP client** [^5] to
...
-   **\[MA\] command**
    ...
    -   In both cases of TCP and UDP, 6-byte MAC address must be
        ...
        -   When users wish to broadcast only Get Request via UDP[^6],
            the relevant field value can be set as the Broadcast MAC
            address[^7]. Users can receive responses from multiple peers.
...
-   The response for ‘get request’ will have the identical format as
    ‘request’ but include the parameter[^9].
...
|----------------------|
    None
**When modifying the IP address format[^10], '.'(dot, 0x2E) must be included such as the
'192.168.11.5'.** 
...
-   Write only command is for controlling the product without the
    parameter[^11].
-   There is a basic command list[^12] and an expanded command list[^13].
-   Settings like IP allocation settings or DNS operation settings via
    command mode[^14] require ‘module reboot’ via **[RT](#rt)** command
...
-   **Meaning:** Data packing of serial interface (Data UART) -
    designated character delimiter[17]
-   **Command Type:** Read / Write
...
[^3]: Command mode switch trigger code via Data UART port

[^4]: Char '+++'

[^5]: Users must use UDP or TCP client because the UDP or TCP server is operating to handle the device commands.

[^6]: Data is sent to the broadcast IP address 255.255.255.255, and data can be sent to all peers in the same network.

[^7]: FF:FF:FF:FF:FF:FF

[^8]: Hex value 0x20 means ‘gap’ in ASCII code.

[^9]: The response for ‘Get Request’ has the same form as ‘Set Request’.

[^10]: IPv4 address forms like IP address, Gateway address, subnet mask, DNS server address, or Remote IP address.

[^11]: Save setting, reboot, switch mode, factory reset and Etc.

[^12]: WIZ107/108SR sync command list, excluding \[UI\] command

[^13]: WIZ750SR exclusive command list

[^14]: When the remote peer address is changed from IP address to domain

[^15]: The received serial data is collected until the designated time is lapsed and will be sent all together via Ethernet. This time starts counting when receiving the serial data stop, and if additional serial data is received before the designated time is lapsed, the count restarts.

[^16]: The received serial data is collected until the designated data length is reached and will be sent all together via Ethernet.

[^17]: The received serial data is collected until the designated character is received and will be sent all together via Ethernet.
